...
from models.LDMAE_DWT_0312 import LDMAE, DiffusionScheduler
...
def train_stage1_vmae(model, dataloader, epochs, device, save_dir, vgg, mean, std, grad_loss_fn, dwt, beta=1e-4, freq='low', resume_path=None):
    ...
    if resume_path is not None:
        if os.path.exists(resume_path):
            ...
            # 1. 모델 가중치 로드 (Multi-GPU 호환)
            if isinstance(model, torch.nn.DataParallel):
                model.module.load_state_dict(checkpoint['model_state'])
            else:
                model.load_state_dict(checkpoint['model_state'])

            # 2. 옵티마이저 상태 로드 (모멘텀 등 유지)
...
def train_stage2_diffusion(model, dataloader, epochs, device, save_dir, scheduler, stage1_weights_path, dwt, freq='low'):
    ...
    if os.path.exists(stage1_weights_path):
        ...
        filtered_state_dict = {k: v for k, v in checkpoint['model_state'].items() if 'dit' not in k}
    ...
    for name, param in model.named_parameters():
        if 'dit' not in name and 'time_mlp' not in name and 'pos_to_latent' not in name and 'cond_encoder' not in name:
    ...
    for epoch in range(epochs):
        ...
        for modal1_img, modal2_img in epoch_bar:
            ...
            noise_target, noise_pred = model(img1_freq, img2_freq, stage=2, timestep=t, scheduler=scheduler)
...
if __name__ == "__main__":
    ...
    save_dir = "LDMAE_DWT_0312_checkpoints"
    os.makedirs(save_dir, exist_ok=True)
    low_pre_model_path = r"C:\Users\12wkd\Desktop\experiments\MMIF\LDMAE_DWT_0312_checkpoints\best_stage1_low_vmae.pth"
    high_pre_model_path = r"C:\Users\12wkd\Desktop\experiments\MMIF\LDMAE_DWT_0312_checkpoints\best_stage1_high_vmae.pth"
    stage1_epochs = 1000
    stage2_epochs = 1000
    ...
    stage1_weights = train_stage1_vmae(
        ...
        dwt=dwt,
        freq='high',
        resume_path=high_pre_model_path
    )
