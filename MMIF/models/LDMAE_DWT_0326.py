...
class LDMAE(nn.Module):
    ...
    def calibrate_scale_factor(self, dataloader, device, dwt_fn, freq='low', num_batches=50):
        ...
        for i, (img1, img2) in enumerate(dataloader):
            ...
            vis_LL, vis_HF_list = dwt_fn(img1)
            ir_LL, ir_HF_list = dwt_fn(img2)
...
class StandardLDMUNet(nn.Module):
    def __init__(self, in_channels=32):
        ...
        up_in_channels = (256, 128)
        up_out_channels = (128, 64)
        ...
        self.cond_proj = nn.Linear(cond_channels, self.context_dim)

        self.conv0 = nn.Conv2d(cond_channels, down_channels[0], 3, padding=1)
        ...
        self.mid_block1 = LDMBlock(down_channels[-1], down_channels[-1], time_emb_dim)
        self.mid_attn = LDM_SpatialTransformer(down_channels[-1], self.context_dim)
        self.mid_block2 = LDMBlock(down_channels[-1], down_channels[-1], time_emb_dim)

        self.ups = nn.ModuleList([])
        self.up_attns = nn.ModuleList([])
        for i in range(len(up_in_channels)):
            self.ups.append(LDMBlock(up_in_channels[i], up_out_channels[i], time_emb_dim, up=True))
            self.up_attns.append(LDM_SpatialTransformer(up_out_channels[i], self.context_dim))

        self.output = nn.Sequential(
            nn.Conv2d(up_out_channels[-1] + down_channels[0], 64, 3, padding=1),
            nn.SiLU(),
    ...
    def forward(self, x, timestep, cond):
        ...
        cond_seq = rearrange(cond, 'b c h w -> b (h w) c')
        ...
        if x.shape[2:] != x_stem_skip.shape[2:]:
            x = F.interpolate(x, size=x_stem_skip.shape[2:], mode='nearest')
