...
class LDMAE(nn.Module):
    def __init__(self, in_channels=3, img_size=224, emb_dim=768, patch_size=4, num_heads=12, encoder_depth=12,
        ...
        self.dit_blocks = nn.ModuleList([
            DiTBlock(emb_dim=latent_dim, num_heads=4, time_emb_dim=time_emb_dim, cond_dim=emb_dim)
        ...
        self.dit_to_noise = nn.Linear(latent_dim, latent_dim)
    ...
    def forward(self, image1, image2, timestep=None, sample_latent=True, latent_scale=1.0, stage=1, scheduler=None):
        ...
        x_sum = torch.abs(image1) + torch.abs(image2)
        if x_sum.ndim == 5:
        ...
        ########################################################
            ...
            cond_feat = self.cond_encoder(image1)
            cond_feat = cond_feat + self.Patch_Posi.pos_embed.to(cond_feat.device)
            ...
            pos_emb_keep = torch.gather(
                self.Patch_Posi.pos_embed.expand(B, -1, -1),
                dim=1,
                index=ids_keep.unsqueeze(-1).repeat(1, 1, self.Patch_Posi.pos_embed.shape[-1])
            )

            pos_emb_keep_latent = self.pos_to_latent(pos_emb_keep)
            z_noisy = z_noisy + pos_emb_keep_latent
        ...
        #
        #     h = w = int(N ** 0.5)
        #     z_spatial = z.reshape(B, h, w, D).permute(0, 3, 1, 2).contiguous()
        #
        ...
        #
        #     cond_img = image1
        #     noise_pred = self.unet(x=z_noisy, timestep=timestep, cond=cond_img)
        #
        #     return noise, noise_pred
...
class StandardLDMUNet(nn.Module):
    def __init__(self, in_channels=1):
        ...
        self.cond_encoder = nn.Sequential(
            nn.Conv2d(cond_channels, 64, 3, padding=1, stride=2),
            nn.SiLU(),
            nn.Conv2d(64, self.context_dim, 3, padding=1, stride=2),
            nn.SiLU()
        )
