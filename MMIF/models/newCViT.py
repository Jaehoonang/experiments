...
class SelfConvAtt(nn.Module):
    ...
    def forward(self, x1, h1, w1, x2, h2, w2, x3, h3, w3):
...
class CrossConvAtt(nn.Module):
    ...
    def forward(self, x1, h1, w1, x2, h2, w2, x3, h3, w3):
        ...
        q3 = rearrange(self.linear_q3(q3), 'b t (h d) -> b h t d', h=self.num_heads)
        k3 = rearrange(self.linear_k3(k3), 'b t (h d) -> b h t d', h=self.num_heads)
        v3 = rearrange(self.linear_v3(v3), 'b t (h d) -> b h t d', h=self.num_heads)

        cross_att_score1 = torch.einsum('bhlk, bhtk ->bhlt', [q1, k3]) * self.scale
        cross_att_map1 = F.softmax(cross_att_score1, dim=-1)

        cross_att_score2 = torch.einsum('bhlk, bhtk ->bhlt', [q2, k3]) * self.scale
        cross_att_map2 = F.softmin(cross_att_score2, dim=-1)

        self_att_score3 = torch.einsum('bhlk, bhtk ->bhlt', [q3, k3]) * self.scale
        self_att_map3 = F.softmax(self_att_score3, dim=-1)

        x1 = torch.matmul(cross_att_map1, v3)
        x2 = torch.matmul(cross_att_map2, v3)
        x3 = torch.matmul(self_att_map3, v3)
...
class newCViTFlow(nn.Module):
    def __init__(self, patch_size, in_channels, embed_dim, dim, num_heads):
        ...
        self.embedding1_3 = ConvEmbedding(7, in_channels, emb_dim[0], stride=4, padding=2, norm_layer=None)
        ################################################################################################################
        ...
        self.embedding2_3 = ConvEmbedding(3, emb_dim[0], emb_dim[1], stride=2, padding=1, norm_layer=None)
        ################################################################################################################
        ...
        self.crossconv3 = CrossConvAtt(emb_dim[2], 6, qkv_bias=False, kernel_size=3, padding_q=1, padding_kv=1, stride=1)

        ################################################################################################################
        self.ffn1_1, self.ffn1_2, self.ffn1_3 = FFN(in_features=emb_dim[0], hidden_features=emb_dim[0]), FFN(in_features=emb_dim[0], hidden_features=emb_dim[0]), FFN(in_features=emb_dim[0], hidden_features=emb_dim[0])
        self.ffn2_1, self.ffn2_2, self.ffn2_3 = FFN(in_features=emb_dim[1], hidden_features=emb_dim[1]), FFN(in_features=emb_dim[1], hidden_features=emb_dim[1]), FFN(in_features=emb_dim[1], hidden_features=emb_dim[1])
        self.ffn3_3 = FFN(in_features=emb_dim[2], hidden_features=emb_dim[2])

        ################################################################################################################
        self.decoder1_1 = TokenDecoder(emb_dim[2], emb_dim[1], emb_dim[1])
        self.decoder2_1 = TokenDecoder(emb_dim[1], emb_dim[0], emb_dim[0])
        self.decoder3_1 = UpConv(emb_dim[0], emb_dim[0]//2)
        self.outconv1 = OutConv(emb_dim[0]//2, 1)
    ...
    def forward(self, x1, x2):
        ...
        x1_3, h3, w3 = self.embedding1_3(x3)

        x1 = rearrange(x1_1, 'b c h w -> b (h w) c')
        x2 = rearrange(x1_2, 'b c h w -> b (h w) c')
        x3 = rearrange(x1_3, 'b c h w -> b (h w) c')
        res1, res2, res3 = x1, x2, x3

        x1, x2, x3 = self.norm1_1(x1), self.norm1_2(x2), self.norm1_3(x3)
        x1, x2, x3 = self.selfconv1(x1, h1, w1, x2, h2, w2, x3, h3, w3)
        x1, x2, x3 = res1+x1, res2+x2, res3+x3
        x1, x2, x3 = self.ffn1_1(x1), self.ffn1_2(x2), self.ffn1_3(x3)

        x3_1_2d = rearrange(x2, 'b (h w) c -> b c h w', h=h3, w=w3)
        ...
        x2_3, h3, w3 = self.embedding2_3(x3)

        x2_1 = rearrange(x2_1, 'b c h w -> b (h w) c')
        x2_2 = rearrange(x2_2, 'b c h w -> b (h w) c')
        x2_3 = rearrange(x2_3, 'b c h w -> b (h w) c')
        res1, res2, res3 = x2_1, x2_2, x2_3

        x1, x2, x3 = self.norm2_1(x2_1), self.norm2_2(x2_2), self.norm2_2(x2_3)
        x1, x2, x3 = self.crossconv2(x1, h1, w1, x2, h2, w2, x3, h3, w3)
        x1, x2, x3 = x1+res1, x2+res2, x3+res3
        x1, x2, x3 = self.ffn2_1(x1), self.ffn2_2(x2), self.ffn2_3(x3)

        x3_2_2d = rearrange(x3, 'b (h w) c -> b c h w', h=h3, w=w3)
        ...
        x3_1, h1, w1 = self.embedding3_1(x1)
        x3_2, h2, w2 = self.embedding3_2(x2)
        x3_3, h3, w3 = self.embedding3_3(x3)

        x3_1 = rearrange(x3_1, 'b c h w -> b (h w) c')
        x3_2 = rearrange(x3_2, 'b c h w -> b (h w) c')
        x3_3 = rearrange(x3_3, 'b c h w -> b (h w) c')
        res1, res2, res3 = x3_1, x3_2, x3_3

        x1, x2, x3 = self.norm3_1(x3_1), self.norm3_2(x3_2), self.norm3_3(x3_3)

        x1, x2, x3 = self.crossconv3(x1, h1, w1, x2, h2, w2, x3, h3, w3)
        x3 = x3 + res3
        x3 = self.ffn3_3(x3)
        ...
        x3 = self.decoder1_1(x3, x3_2_2d)
        x3 = self.decoder2_1(x3, x3_1_2d)
        x3 = self.decoder3_1(x3)
        x3 = self.outconv1(x3)
        x3 = F.interpolate(x3, size=og_x3.shape[-2:], mode="bilinear", align_corners=False)
