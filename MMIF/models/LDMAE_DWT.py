...
class LDMAE(nn.Module):
    def __init__(self, in_channels=3, img_size=224, emb_dim=768, patch_size=16, num_heads=12, encoder_depth=12,
        ...
        self.dwt = DWTForward(J=1, mode="periodization", wave='haar')
        # VMAE module
    ...
    def forward(self, image1, image2, timestep=None, sample_latent=True, freq='low', latent_scale=1.0, stage=1, scheduler=None):
        img1_low, img1_high = self.dwt(image1)
        img2_low, img2_high = self.dwt(image2)

        if freq=='low':
            x = torch.abs(img1_low) + torch.abs(img2_low)
            x = self.Patch_Posi(x)

            score = compute_focus_score(img1_low, img2_low, patch_size=self.Patch_Posi.patch_size)
            mask, ids_keep, ids_mask, ids_restore = focus_mask(score, mask_ratio=0.3)

            x_vis = apply_focus_mask(x, ids_keep)

        else:
            dal1_High = img1_high[0] if isinstance(img1_high, (list, tuple)) else img1_high
            dal2_High = img2_high[0] if isinstance(img2_high, (list, tuple)) else img2_high

            dal1_High = torch.abs(dal1_High).squeeze(1)
            dal2_High = torch.abs(dal2_High).squeeze(1)

            x = dal1_High + dal2_High
            x = self.Patch_Posi(x)

            dal1_avg = dal1_High.mean(dim=1, keepdim=True)
            dal2_avg = dal2_High.mean(dim=1, keepdim=True)

            score = compute_focus_score(dal1_avg, dal2_avg, patch_size=self.Patch_Posi.patch_size)
            mask, ids_keep, ids_mask, ids_restore = focus_mask(score, mask_ratio=0.3)
        ...
        if stage == 1:
            ...
            if freq == 'low':
                return x_out, mask, posterior, img1_low, img2_low
            else:
                return x_out, mask, posterior, dal1_High, dal2_High
            ...
            cond_patches = self.Patch_Posi(cond)
